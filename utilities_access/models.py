...
CURR_DATA_DIR = os.path.join(os.path.dirname(__file__), 'models_param')
...
def get_latest_sign_id():
    # 查询数据库获取当前序号
    access_lock.acquire()
    file_ = open(os.path.join(CURR_DATA_DIR, 'latest_sign_id'), 'r+b')
...
def acquire_latest_sign_id():
    ...
    file_ = open(os.path.join(CURR_DATA_DIR, 'latest_sign_id'), 'w+b')
